...
export class CustomaryCookbookIndexGenerator {
    static async generateResource() {
        ...
        const tests = readdirSync(src_dir, { recursive: true })
            .filter(f => test_ts_file_name_pattern.test(f));
        const content = await toIndexString(tests, new PageHtmlContentLoaderLocalFile());
        customary_cookbook_index_json_local_write(content);
    }
}
async function toIndexString(tests, page_html_content_loader) {
    const data_structure = await Promise.all(tests.sort().map(line => to_entry(line, page_html_content_loader)));
...
//# sourceMappingURL=CustomaryCookbookIndexGenerator.js.map