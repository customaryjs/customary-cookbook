...
describe(suite.title, async function () {
    ...
    describe('happy day', async function () {
        it('looks good', async function () {
            this.retries(64);
            const element = CT.querySelector('logic-classmap-and-stylemap-within-for-within-if-recipe', window);
            CT.spot('Water the plants', element, { selectors: 'span' });
            CT.spot('Sweep the floor', element, { selectors: 'span' });
            const span = CT.querySelector('span', element);
            chai.assert.isTrue(span.classList.contains('done'));
            chai.assert.strictEqual(span.style.color, 'green');
        });
    });
});
//# sourceMappingURL=test-Customary-logic-classMap-and-styleMap-within-for-within-if.js.map