...
describe(suite.title, async function () {
    ...
    describe('happy day', async function () {
        ...
        it('submit both forms', async function () {
            this.retries(64);
            (CT.querySelector('button', silent) as HTMLButtonElement).click();
            (CT.querySelector('button', announcing_form) as HTMLButtonElement).click();
        });

        it('the shadowRoot keeps the submit event to itself', async function () {
            this.retries(64);
            chai.assert.strictEqual(
                CT.allTextContent(CT.querySelector('span.outcome', silent)),
                '',
